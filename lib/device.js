...
class Device extends EventEmitter {
  constructor (socket, portInfo) {
    ...
    this.serial.on('data', data => {
      ...
      this.socket.emit('serial:data', this.serial.path, str)
      this.emit('data', str)
    })
    this.serial.on('open', err => {
      if (err) {
        ...
        this.socket.emit('serial:error', this.serial.path, err)
        this.emit('error', err)
      } else {
        console.log('serial:open - ', this.serial.path)
        this.isOpen = this.serial.isOpen
        this.socket.emit('serial:open', this.serial.path)
    ...
    this.serial.on('close', err => {
      if (err) {
        ...
        this.socket.emit('serial:close', this.serial.path)
        this.emit('close')
      }
    })
    this.isOpen = this.serial.isOpen
  ...
  upload (code) {
    ...
    protocol.write(this.serial, code, (err, result) => {
      if (err) {
        ...
        this.socket.emit('serial:error', this.serial.path, 'Upload failed')
        ...
        setTimeout(() => {
          ...
          this.socket.emit('serial:upload', this.serial.path)
