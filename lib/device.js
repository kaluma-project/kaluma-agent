...
class Device {
   ...
   * @param {function(err:Error,version:string)} cb
   */
  requestFirmwareVersion (cb) {
    ...
    const dataHandler = (data) => {
      ...
      buffer = buffer + stripEscapes(s)
      const prefix = '\x12['
      const postfix = ']\x14\r\nundefined\r\n'
      if (buffer.includes(postfix)) { // detect end of version
        this.serial.off('data', dataHandler)
        this.session.bind(this)
        var sp = buffer.indexOf(prefix)
        var ep = buffer.indexOf(postfix)
        if (sp >= 0 && ep >= 0) {
          // var pre = buffer.substr(0, sp)
          var rest = buffer.substr(ep + postfix.length)
          if (this.session.socket) {
            this.session.socket.emit('event:data', this.comName, rest)
          }
          var version = buffer.substr(sp + 2, ep - sp - 2)
          if (semver.valid(version)) {
            if (cb) cb(null, version)
          } else {
            if (cb) cb(new Error('Invalid version number'))
          }
        } else {
          if (cb) cb(new Error('Failed to get firmware version'))
        }
      }
    }
    this.session.unbind(this)
    this.serial.on('data', dataHandler)
    this.serial.write(`\rconsole.log('\x12' + '[' + process.version + ']' + '\x14')\r`)
  }
  ...
  checkForUpdate (cb) {
    if (this.serial.isOpen) {
      this.requestFirmwareVersion((err, version) => {
        if (err) {
          if (cb) cb(err)
        } else {
          const options = {
