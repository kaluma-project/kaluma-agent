...
class Agent extends EventEmitter {
  constructor () {
    ...
    io.on('connection', socket => {
      ...
      this.socket = socket
      this.socket.on('serial:scan', () => { this.scan() })
      this.socket.on('serial:open', (port) => { this.open(port) })
      this.socket.on('serial:close', (port) => { this.close(port) })
      this.socket.on('serial:write', (port, data) => { this.write(port, data) })
      ...
      if (err) {
        ...
        portInfoArray.forEach(portInfo => {
          if (Device.isSupported(portInfo)) {
            ...
            devices[device.comName] = device
          }
        })
        // Check disconnected ports
        for (let key in this.availableDevices) {
          if (!devices[key]) {
            losts.push(this.availableDevices[key])
          }
        }
        // Check newly connected ports
        for (let key in devices) {
          if (!this.availableDevices[key]) {
            founds.push(devices[key])
          }
        }
   ...
   */
  open (comName) {
    var device = this.availableDevices[comName]
    if (device) {
      device.open()
    }
  }
