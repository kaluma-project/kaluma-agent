...
function parseCondensedVersion (condensedVersion) {
  var semver = ''
  semver += parseInt(condensedVersion[0], 16)
  semver += '.' + parseInt(condensedVersion[1], 16)
  semver += '.' + parseInt(condensedVersion[2], 16)
  if (condensedVersion[3] === 'A') {
    semver += '-alpha.' + parseInt(condensedVersion[4], 16)
  } else if (condensedVersion[3] === 'B') {
    semver += '-beta.' + parseInt(condensedVersion[4], 16)
  } else if (condensedVersion[3] === 'R') {
    semver += '-rc.' + parseInt(condensedVersion[4], 16)
  }
  return semver
}
...
function toDeviceObject (portInfo) {
  ...
  if (info) {
    Object.assign(portInfo, info)
    // Extract firmwareVersion from serialNumber
    if (portInfo.serialNumber && portInfo.serialNumber.split('-').length === 2) {
      var tokens = portInfo.serialNumber.split('-')
      var firmwareVersion = parseCondensedVersion(tokens[0].trim())
      var actualSerialNumber = tokens[1].trim()
      portInfo.firmwareVersion = firmwareVersion
      portInfo.serialNumber = actualSerialNumber
    }
    return portInfo
  }
  return null
